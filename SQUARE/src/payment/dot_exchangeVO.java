package payment;

import java.io.IOException;
import java.text.SimpleDateFormat;
import java.util.Date;

public class dot_exchangeVO {
	private int de_num;
	private String de_id;
	private int de_value;
	private int de_status;
	private Date de_date;
	private Date de_dated;
	
	public int getDe_num() {
		return de_num;
	}
	public void setDe_num(int de_num) {
		this.de_num = de_num;
	}
	public String getDe_id() {
		return de_id;
	}
	public void setDe_id(String de_id) {
		this.de_id = de_id;
	}
	public int getDe_value() {
		return de_value;
	}
	public void setDe_value(int de_value) {
		this.de_value = de_value;
	}
	public int getDe_status() {
		return de_status;
	}
	public void setDe_status(int de_status) {
		this.de_status = de_status;
	}
	public Date getDe_date() {
		return de_date;
	}
	public void setDe_date(Date de_date) {
		this.de_date = de_date;
	}
	public Date getDe_dated() {
		return de_dated;
	}
	public void setDe_dated(Date de_dated) {
		this.de_dated = de_dated;
	}
	
}	
