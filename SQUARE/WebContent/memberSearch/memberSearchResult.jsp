...
<s:if test="searchKind == 0">
	<span class="resultKeyword"><s:property value="search" /></span> <span class="resultTest">통합검색결과</span><br>
	
	<span class="resultKind">
		게시판(제목)
		 <s:if test="boardSubjectListsum > 9"><span class="moreViewSpan"><a href="memberSearchMoreView.action?k=bs&key=<s:property value="search" />">검색결과 더보기 (<s:property value="boardSubjectListsum" />건)</a></span></s:if>
	</span>
	...
	<s:else>
		<s:iterator value="boardSubjectList" status="stat">
			...
			<span class="resultSubtext"><s:property value="board_author" /> | <s:property value="boardSubjectLN.get(#stat.index)" /> 라인 | 조회수 <s:property value="board_readcount" /> | 작성일 <s:property value="board_regdate" /></span>
		</s:iterator>
	</s:else>
	<span class="resultKind">
		게시판(내용)
		<s:if test="boardContentListsum > 9"><span class="moreViewSpan"><a href="memberSearchMoreView.action?k=bc&key=<s:property value="search" />">검색결과 더보기 (<s:property value="boardContentListsum" />건)</a></span></s:if>
	</span>
	...
	<s:else>
		<s:iterator value="commentContentList" status="stat">
			<a href='viewAction.action?line_num=<s:property value="line_num" />&board_num=<s:property value="board_num" />&currentPage=1'><s:property value="comment_content" /></a> <br>
...
<s:elseif test="searchKind == 1">
	...
	<span class="resultKind">
	게시판(작성자)
	<s:if test="boardWriterListsum > 9"><span class="moreViewSpan"><a href="memberSearchMoreView.action?k=bw&key=<s:property value="search" />">검색결과 더보기 (<s:property value="boardWriterListsum" />건)</a></span></s:if>
	</span>
